...
| S016 | 2026-07-14 | **Ampliación desde desvío H17→H7 (ver `ENTERPRISEBOT_ATTACHED_MILESTONE_V07.md`, fila S016, para el contexto completo).** Corrige un error propio de la misma sesión: se había creado por accidente `fleet.views.MachineHistoryView` duplicada (URL/plantilla/sidebar propios) sin comprobar antes que esta vista ya existía — revertida por completo, sin restos huérfanos, y fusionado el trabajo real aquí. `MachineHistoryView` (`history/views.py`): mixin ampliado de `WorkshopRequiredMixin` a `CompanyUserRequiredMixin` (sin restricción de rol) — confirmado explícitamente por Miguel Ángel: al ser de solo lectura, accesible a ADMIN, SUPERVISOR, WORKSHOPBOSS, WORKSHOP y DRIVER por igual, no solo WORKSHOP/ADMIN. Nueva sección "Libro de revisiones — fichas de avería" añadida a `panel/templates/panel/machine_history.html`, bajo la tabla de intervenciones ya existente: historial completo de `BreakdownTicket` de la máquina seleccionada (independiente del rango de fechas del formulario de intervenciones), cada uno con su galería combinada de fotos — `photos` (JSONField, WhatsApp) + `task_photos` (H7, `TaskPhoto`). Sidebar (`_nav_items.html`): gate del subgrupo "Centro de gasto" y del ítem "Historial de Máquina" ampliado de WORKSHOP/ADMIN a `!= ASSISTANCE` (incluye DRIVER); "Centros de gasto" (gestión) conserva su gate interior ADMIN/SUPERVISOR/WORKSHOPBOSS sin cambios. Un push sin migración con la vista duplicada mal reexportada rompió el despliegue real (`ImportError` en `manage.py check`, confirmado con datos del servidor) — corregido y reverificado en el mismo tramo. |

---

### Hoja de Ruta para la Siguiente Sesión

#### TRABAJO COMPLETADO EN S053 + S056 + S016
...
Mis partes. Alias de chat IRC eliminado de own_profile.html. S016:
acceso ampliado a todos los roles no-ASISTENCIA + libro de revisiones
(tickets + fotos) añadido a la misma vista.
