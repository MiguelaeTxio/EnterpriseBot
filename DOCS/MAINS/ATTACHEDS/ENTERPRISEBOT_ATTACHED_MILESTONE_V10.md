...
**Pendiente para cuando se retome H10:** el Paso 8 (persistencia real en
la nube M365) -- sin acción posible hasta que Miguel Ángel tenga
novedades del administrador de Grupo Álvarez tras la reunión
pendiente. **Añadido en S013 (Bloque B, ver fila S013 arriba):**
formato de fecha DD/MM/AAAA en el correo de confirmación
(`spare_parts/tasks.py`, hoy en ISO); constraint de unicidad por
`delivery_number` (`UniqueConstraint` en `DeliveryNote`, condicional a
`delivery_number != ''`, mismo patrón que `Supplier.tax_id` -- BD ya
verificada sin duplicados tras el borrado de S013, migración limpia de
aplicar); conversión del flujo de subida de síncrono a asíncrono vía
Celery (`DeliveryNoteUploadView.post()` bloquea hoy la petición
esperando a Gemini).
