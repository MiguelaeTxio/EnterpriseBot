...
de Sesiones, abajo, para el detalle técnico completo de los 3
commits: `91587bf`, `f425986`, `ed15d69`). Los 6 puntos de la
especificación de Miguel Ángel están implementados: código único
obligatorio por albarán completo, leído exclusivamente del campo
Observaciones/Notas impreso por el proveedor, sin tolerancia a mano,
con rechazo automático y motivo preciso (código ausente, código no
reconocido, o código detectado como artículo en vez de en
Observaciones) en vez de resolución silenciosa. Además, a petición
explícita de Miguel Ángel durante la propia sesión, se eliminó por
completo la corrección manual de CUALQUIER dato del albarán (no solo
el código de máquina) — la pantalla de revisión pasa a ser de solo
confirmación/rechazo, nunca de edición, para forzar fotos legibles en
vez de depender de arreglos manuales.

**Sin nada más pendiente en la hoja de ruta propia de H10** — el
hito quedó CERRADO COMPLETO ya en S009 (ver "CIERRE DE HITO — H10
PASA A PAUSADO" arriba) salvo este primer punto, ahora resuelto. Si
Miguel Ángel quiere retomar H10 en el futuro, será por trabajo nuevo
a definir en su momento, no por nada anotado aquí.
...
|--------|-------|-----------------|---------|
| S030 | 2026-07-23 | Modal global "Añadir repuesto" (rescate de repuestos en el limbo desde cualquier tarea) + corrección de 15 SparePartLine sin máquina | **Sesión con H23 EN PROGRESO durante toda la sesión (ver ese anexo) y desvío puntual a H10 -- Caso A, sin PCH, marcador sin mover.** Origen: pregunta de Miguel Ángel durante H23 sobre cómo un operario puede añadir repuestos no vinculados a la máquina desde el propio parte digital. Investigación confirmó dos mecanismos ya existentes, ambos acotados a la máquina de la tarea concreta (`TaskTicketResolutionView` para pre-asignados, `SparePartWarehouseSearchView` para almacén) -- ningún sitio buscaba repuestos `PRE_ASSIGNED` de OTRAS máquinas, dejándolos invisibles para siempre si nunca se consumían en su máquina de origen ("el limbo", terminología ya presente en el docstring de `SparePartEntry`). Nuevo `StockAssignmentService.search_global()` (combina `WAREHOUSE`+`PRE_ASSIGNED` de toda la empresa) + `SparePartGlobalSearchView`/URL `buscar-global/` (endpoint HTMX de solo lectura, funciona antes de guardar el parte, sin `WorkOrderEntryLine` todavía) + botón "Repuesto de almacén / pre-asignado" en `form_entry.html` con selector de tarea destino (número + código de máquina) y cantidad/nivel por operario para los de almacén (los pre-asignados se consumen enteros, como ya funcionaba). Backend: nuevo parseo `entrada_{i}_consume_warehouse_{pk}_qty/_level` en `_parse_entry_lines_from_post` (mismo patrón que `consume_part_pks` ya existente), materializado en la misma transacción que crea la tarea vía `StockAssignmentService.consume_from_warehouse()` (sin cambios en ese método). **Corregido en el mismo bloque, hallazgo real de una tabla comparativa:** el diseño inicial usaba una tabla de 5 columnas dentro de un modal -- aviso real de Miguel Ángel probando en el propio móvil, campo de cantidad inalcanzable sin scroll horizontal; rediseñado a lista apilada, mismo patrón que `_warehouse_search_results.html`. **Fuera de la hoja de ruta, corregidos con datos reales (directriz 4.9):** durante la misma investigación se encontró que el selector "Bloque asociado" de repuestos ya guardados al editar un parte no estaba conectado a nada (huérfano, ni JS ni backend lo procesaban) -- corregido en `panel/views_operator.py` (ver anexo H07, `S_H07_14`, mismo bloque que el bug de pérdida de datos de horario/dieta/pausa, mismo patrón de causa raíz). Auditoría de producción sobre los `SparePartLine` con `vehicle=NULL` en tareas con máquina real: 15 encontrados, corregidos con script dedicado (vehículo asignado desde la máquina real de la propia `entry_line`, verificados uno a uno antes de tocar cada fila). Verificado: `py_compile`/`node --check` limpios en los archivos tocados. Sin migración. |
| S020 | 2026-07-15 | Incidencia real fuera de la hoja de ruta original: código de máquina delimitado en línea + fallback repuesto/stock/almacén | **Sesión con trabajo directo en H24 (EN PROGRESO, ver ese anexo) y desvío puntual a H10 — Caso A, sin PCH, marcador `EN PROGRESO` sin mover.** Miguel Ángel planteó en producción que no todos los proveedores anotan el centro de gasto en Observaciones/Notas de sus albaranes. Ampliada la extracción Gemini Vision (`spare_parts/services.py`, commit `2aa236c`): `general_machine_code_raw` ahora se busca en DOS fuentes por orden de preferencia -- (1) Observaciones/Notas (como hasta S015), (2) un código delimitado por almohadilla, asterisco o comillas -- el mismo carácter antes Y después -- dentro de la descripción de una línea de artículo (antes disparaba rechazo automático vía `document_code_found_as_line_item`; ahora es fuente válida). Un fragmento sin delimitar correctamente sigue provocando rechazo, sin cambios ahí. **Nuevo fallback de palabra clave (solo si no hay código en ninguna de las dos fuentes):** si el texto del albarán menciona "repuesto", "stock" o "almacén", nuevo campo `general_warehouse_keyword_found=true` y el documento se asigna a `WAREHOUSE` (almacén interno de repuestos sin asignar a máquina) -- destino único para todo el albarán, S015 sigue vigente (nunca uno distinto por línea). **Decisión explícita de Miguel Ángel, confirmada en dos rondas de preguntas del modelo antes de tocar código (directriz 4.8):** de momento TODO lo marcado por palabra clave va al mismo `WAREHOUSE` sin diferenciar por taller/familia (Mecánico/Elevación/Huelva) -- no se inventa ningún centro de gasto `EMPRESA_ALMACEN_GENERAL` nuevo; esa diferenciación queda pospuesta a una sesión futura ("todavía no tengo muy claro lo que vamos a hacer con Huelva"). Nueva función `resolve_document_assignment()` combina código + fallback con la prioridad correcta, usada tanto en la extracción en background (`spare_parts/tasks.py`) como en `validate_document_assignment()` (`spare_parts/views.py`, sin cambio de firma). Textos de ayuda y mensajes de rechazo actualizados en `delivery_note_detail.html` y `spare_parts/views.py`. Sin migración (ningún cambio de `models.py`). Desplegado y verificado en verde (`run 29414473442`), incluido el reinicio condicional automático del worker Celery (id `242133`, disparado por el cambio en `spare_parts/tasks.py`, confirmado con el resumen real del Action, no solo por el patrón de archivos). |
| S015 | 2026-07-13 | Primer punto obligatorio COMPLETADO (asignación única por albarán, sin corrección manual, rechazo obligatorio); bug real de OAuth corregido; backfill de 8 albaranes históricos con foto | **Sesión con trabajo directo previo en H17 (Bloque C completo, ver ese anexo, fila S015) y desvío posterior a H10 — Caso A, sin PCH, marcador `EN PROGRESO` sin mover.** **Primer punto de la hoja de ruta, CERRADO (commit `91587bf`):** reescrito el esquema de extracción Pydantic (`DeliveryNoteLineExtraction` sin `machine_code_raw` por línea -- confirmado por Miguel Ángel como remanente sin sentido de negocio, "el código por línea deja de tener sentido, la asignación es completa por albarán"; `DeliveryNoteExtraction.general_machine_code_raw` reescrito de opcional/tres-fuentes a único/obligatorio, solo desde el campo impreso Observaciones/Notas; nuevo `document_code_found_as_line_item: bool` para el caso "código como artículo"); prompt de extracción reescrito de raíz; nueva `services.validate_document_assignment()` (motivo de rechazo preciso en castellano) usada tanto en `DeliveryNoteDetailView` como en `DeliveryNoteConfirmView` (defensa en profundidad); `extract_delivery_note_data()` (tasks.py) resuelve el código una sola vez y lo aplica a TODAS las líneas. **Ampliación no prevista en la hoja de ruta original, a petición explícita de Miguel Ángel durante la sesión:** eliminada la corrección manual de CUALQUIER dato del albarán (no solo el código) -- `DeliveryNoteDetailView.post()` retirado por completo, la pantalla pasa a ser de solo confirmar/rechazar; nueva `DeliveryNoteRejectView` (borra el intento + archivo origen, redirige a subir una foto nueva -- "si no confirma, hay que volver a subir la foto", decisión de borrar en vez de introducir un estado `REJECTED` con migración, confirmada correcta por Miguel Ángel). `DeliveryNoteLine.machine_code_raw` conservado como campo LEGACY sin migración (solo `help_text`/`verbose_name`), a petición expresa de Miguel Ángel para no generar una migración solo para eso. **WORKSHOPBOSS como destino asignable eliminado (commit `6717375`, ver también anexo H17 S015):** confirmado por Miguel Ángel que era un remanente del diseño original del H14 sin tickets reales afectados -- `operators_qs`/`assignable_operators`/validación final de `assignee_pk` restringidos a WORKSHOP. **Backfill de 8 albaranes históricos huérfanos de foto (correo electrónico, previos a la persistencia en Drive de S014), commit `f425986`:** nuevo management command `attach_delivery_note_photo` (dry-run por defecto, mapeo `PK|NOMBRE_ARCHIVO`, guarda de seguridad contra sobrescritura, reutiliza `upload_delivery_note_photo_to_drive.run()` síncrono) -- los 8 (pks 12,13,15,17,19,20,21,22) identificados por consulta ORM + cotejo visual foto a foto contra el listado, y subidos a Drive con éxito, verificado con datos reales de BD (`drive_file_id` presente en los 8), no solo con el log (silencioso por la incidencia ya conocida de S010, sin `LOGGING` explícito). **Bug real de OAuth encontrado y corregido en producción (commit `ed15d69`):** la autorización de un solo uso de Google Drive (construida en S014) nunca se había podido completar -- `InvalidGrantError: Missing code verifier` en `/panel/gdrive/oauth-callback/`. Causa raíz verificada en línea contra la documentación actual de `google-auth-oauthlib` (Directriz 4.4 -- conocimiento de entrenamiento desactualizado sobre el comportamiento por defecto de la librería): `Flow.from_client_config()` activa PKCE por defecto en la versión instalada, generando un `code_verifier` que solo vive en el objeto `Flow` de esa petición HTTP concreta; como `GDriveAuthorizeView` y `GDriveOAuthCallbackView` son dos peticiones independientes sin sesión compartida, el segundo paso nunca podía tenerlo. Fix: `autogenerate_code_verifier=False` explícito en `_build_flow()` (la app es un cliente confidencial con `client_secret` propio, no necesita PKCE). Autorización completada por Miguel Ángel tras el fix, las 4 variables de entorno (`GDRIVE_OAUTH_CLIENT_ID/SECRET/REFRESH_TOKEN`, `GDRIVE_ROOT_FOLDER_ID`) confirmadas presentes. **Incidencia de despliegue sin relación con el código:** el deploy del commit `f425986` falló en el paso "Recargar la web app" (curl exit code 22, el ya documentado 409 `slow_startup` de PythonAnywhere) -- verificado vía API de GitHub (`api.github.com/repos/.../actions/runs`, jobs, steps) que el paso previo (`git pull`+`migrate`+`collectstatic`) sí había tenido éxito, y que el despliegue siguiente (`ed15d69`) completó su propio reload con éxito, dejando el servidor sincronizado con `HEAD` sin ninguna acción manual adicional. Sin migración en ninguno de los 3 commits de esta sesión (solo `help_text`/`verbose_name` en `models.py`). |
