...
def build_live_config(
    ...
    # ------------------------------------------------------------------
    # STEP 4C — Build Alia Mechanic Expert Context + Breakdown Greeting
    #           Construir Contexto de Alia Mecanica Experta + Saludo Averia
    ...
    if caller_number:
        try:
            _internal_contact = Contact.objects.filter(
                company=company,
                phone_number=caller_number,
            ).first()
            if _internal_contact:
                ...
                alia_mechanic_context = (
                    ...
                    "(base o ruta). Si la propia descripcion del fallo ya "
                    "revela en que parte de la maquina esta (p.ej. "
                    "'intermitentes del lado izquierdo' ya implica que la "
                    "ubicacion de la averia es el lado izquierdo), NO "
                    "preguntes por separado 'en que parte de la maquina se "
                    "encuentra la averia' -- ya lo sabes, infierelo "
                    "directamente de lo que ha dicho el conductor. Pregunta "
                    "la ubicacion en la maquina UNICAMENTE cuando genuinamente "
                    "no se pueda deducir de la descripcion del fallo (p.ej. "
                    "'pierde aceite hidraulico' sin mas detalle). Nunca "
                    "preguntas la urgencia: la calculas tu misma segun el "
                    "fallo.\n\n"
                    ...
                    "REGLA INAMOVIBLE: menciona la palabra 'WhatsApp' de "
                    "forma explicita CADA VEZ que pidas la ubicacion -- "
                    "nunca digas solo 'a este numero' o 'a este mismo "
                    "numero' sin la palabra WhatsApp, porque el conductor "
                    "puede confundirlo con esta misma llamada de voz. "
                    "Puedes parafrasear la frase, pero la palabra WhatsApp "
                    "y la aclaracion de que es un canal distinto a esta "
                    "llamada son obligatorias siempre. "
                    ...
                    "se encuentra?' -- (NOTA: no pregunta 'en que parte de "
                    "la maquina', porque 'lado izquierdo' ya lo dijo el "
                    "conductor.)\n"
                    ...
                    "no menciona WhatsApp, el conductor puede pensar que es "
                    "este mismo canal de voz.\n"
                    "'Responda a este numero con su ubicacion' -- INCORRECTO: "
                    "misma razon, falta la palabra WhatsApp.\n"
                    "'Puede circular con precaucion?' -- INCORRECTO: con un "
                    "fallo de senalizacion la respuesta es no, no se pregunta.\n"
                    "'Y en que parte de la maquina esta la averia?' tras un "
                    "conductor que ya dijo 'lado izquierdo' o similar -- "
                    "INCORRECTO: pregunta redundante, la ubicacion ya se "
                    "infiere de la descripcion del fallo.\n\n"
                ...
                breakdown_initial_greeting = (
                    ...
                    f"la descripción de la avería "
                    f"y su ubicación (base o ruta). "
                    f"Pídele que comience cuando quiera. "
                    f"Mantén un tono profesional y directo."
                )
                logger.info(
                    "[CONFIG] Llamante '%s' es Contact interno ('%s') — "
                    "perfil Alia y saludo de averia activados.",
                    caller_number,
                    _caller_display,
                )
    ...
    if breakdown_initial_greeting:
        ...
        logger.info(
            "[CONFIG] initial_greeting personalizado de averia activado "
            "para el llamante '%s'.",
            caller_number,
        )
    else:
